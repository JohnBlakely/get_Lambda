...
function ch17(p, TM)
    ...
    rescale_factor = p.rescaling.RF_mScale^(-5/2) * p.rescaling.RF_aScale^-4
    return rescale_factor * 4.4e-10 * (TM/ p.rescaling.fl)^0.35 * exp(-102000 * p.rescaling.fl /TM) *p.artificial[11]
end
...
function ch20(p, TM)
    ...
    return p.rescaling.RF_aScale^(-1) *p.rescaling.RF_mScale^(-2) * 4.6e-6/1e6 * (TM/p.rescaling.fl)^(-0.65)* p.artificial[12]
end
#H_2 + H -> 3H (from Hollenback and McKee)
#function h2hdis(p, TM)
#        rescale_factor = p.rescaling.RF_mScale^(-3/2) * p.rescaling.RF_mxScale^(-1/2) * p.rescaling.RF_aScale^(-1)* p.artificial[12]
#    return rescale_factor * 3.4e-9/1e6 * exp((8000 - 5.19e4) * p.rescaling.fl/TM)
#end
...
function h2hdis(p, NH, T)
    ...
    return overall_scaling * rate/1e6 * p.artificial[13]
end
...
function threeh_association(p, TM)
    ...
    return 1e-12 *p.rescaling.RF_mxScale^-1 * p.rescaling.RF_mScale^-4 * p.rescaling.RF_aScale^-4 * (6e-32 * (TM/p.rescaling.fl)^(-1/4) + 2e-31 *(TM/p.rescaling.fl)^(-1/2))* p.artificial[14]
end
...
function h_h_h2_association(p, TM)
    ...
    return 1e-12 *p.rescaling.RF_mxScale^-1 * p.rescaling.RF_mScale^-4 * p.rescaling.RF_aScale^-4 * (1/8) *(6e-32 * (TM/p.rescaling.fl)^(-1/4) + 2e-31 *(TM/p.rescaling.fl)^(-1/2))* p.artificial[15]
end
...
function h_h_hp_to_h2p(p,TM)
   ...
   #statistical weight * (direct association + charge transfer) * (cm^6 to m^6)
   return 0.25* (kda + kcta)* 1e-12 * 1e-29 * p.rescaling.RF_mxScale^-1 * p.rescaling.RF_mScale^-4 * p.rescaling.RF_aScale^-4 * p.artificial[17]
